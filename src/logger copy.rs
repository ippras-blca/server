...
pub(crate) async fn serve(mut temperature_receiver: Receiver<Value>) {
    loop {
        if let Err(error) = run(&mut temperature_receiver).await {
            if let Error::Receiver(error) = error {
...
pub async fn ipc(receiver: &mut Receiver<Value>) -> Result<()> {
    ...
    let mut path = PathBuf::from(format!("{}.log.ipc", today.format(FILE_FORMAT)));
    ...
    loop {
        ...
        loop {
            let (identifiers, temperatures, date_time) = receiver.recv().await?;
        ...
        if let Some(tomorrow) = tomorrow.take() {
            ...
            //     Ok(())
            // });
            path = PathBuf::from(format!("{}.ipc.log", today.format(FILE_FORMAT)));
        }
    }
}
